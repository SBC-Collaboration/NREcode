...
def prep(topdir_searchlocations):
    ...
    fidfun_list = [lambda pos, hv: np.ones(hv.shape, dtype=bool)
                   for exp in experiment_list]
    
    vetofun_list = [lambda pos, hv: np.zeros(hv.shape, dtype=bool)
                    for exp in experiment_list]
...
def XeSBCcal_post(theta, whichnuisance=np.ones(n_nuisance, dtype=bool)):
...
def XeSBCcal_post_decomp(theta, whichnuisance=np.ones(n_nuisance, dtype=bool)):
    ''' Posterior log-likelihod function in this module
    ...
    for i_exp in range(len(experiment_list)):
        ...
        bub_mult = np.cumsum(np.ones(nu.shape, dtype=np.int32), axis=1)
        bub_keep = bub_mult <= neutron_data[i_exp]['max_mult'][:, np.newaxis]
        total_chisq[i_exp] = np.sum(this_chisq[bub_keep].ravel())
...
def CheckParametrization(Epts): 
    ...
    if np.any(np.abs(Epts.ravel()) > 1000): 
        #print('e3')
        return False
...
def EfficiencyInterpolation_2Dmesh(E_r, s, E_T, Epts):
    ...
    # check param, return inf if invalid
