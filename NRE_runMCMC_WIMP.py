# -*- coding: utf-8 -*-
"""
Created on Mon Sep 28 07:10:03 2020
...
for i_epoch in epoch_list:
    ...
    for i_dim in range(mDim):
        for i_bin in range(bin_number):
            ...
            b = np.max(wimp_samples_epoch[:,i_dim])
            a = np.min(wimp_samples_epoch[:,i_dim])
            
            bin_size = (b-a)/bin_number
            
            index = np.asarray(np.where((wimp_samples_epoch[:,i_dim] >= a + i_bin*bin_size) & (wimp_samples_epoch[:,i_dim] <a + (i_bin+1)*bin_size)))
            unique, unique_indices = np.unique(log_prob_epoch[index[0,:]],return_index=True)
            
            if unique.size != 0:    
                epoch_starting_points = np.vstack((epoch_starting_points,samples_epoch[index[0,unique_indices[-1]]])) 
    ...
    vol_epoch = calcVol2(wimp_samples,log_prob)	#additive constant from historical PICO NR fit
